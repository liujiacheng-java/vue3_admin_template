<template>
    <div>
        ...
        <el-card style="margin:20px 0; ">
            <div v-show="scene == 0">
                <el-button @click="scene = 1" type="primary" icon="Plus"
                ...
                <el-table style="margin:20px 0" border :data="attrList">
                    ...
                    <el-table-column label="属性值名称">
                        <template #default="{ row, $index }">
                            <el-tag v-for="(item, index) in row.attrValueList" :key="index" style="margin: 5px;">{{
                                item.valueName }}</el-tag>
                        </template>
                    </el-table-column>
                    <el-table-column label="操作" width="200px">
                        <template #default="{ row, $index }">
                            <el-button type="success" icon="Edit" @click="updateAttr(row)" />
                            <el-popconfirm :title="`你确定删除${row.attrName}吗?`" width="200px" @confirm="removeAttr(row)">
                                <template #reference>
            ...
            <div v-show="scene == 1">
                ...
                <el-table border style="margin:10px 0px" :data="attrParams.attrValueList">
                    ...
                    <el-table-column label="属性值名称">
                        <template #default="{ row, $index }">
                            <el-input :ref="(element: any) => {
                                inputRefs[$index] = element
                            }" @blur="toLook(row, $index)" v-if="row.flag" v-model="row.valueName"></el-input>
                ...
                <el-button type="primary" size="default" :disabled="attrParams.attrValueList.length == 0"
                    @click="saveAttr"> 保存
                </el-button>
                <el-button type="primary" size="default" @click="scene = 0">取消</el-button>
            </div>
...
//获取对应分类下已有的属性与属性值接口
const getAttr = async () => {
    let res = await reqAttr(categoryStore.c1Id, categoryStore.c2Id, categoryStore.c3Id)
    // console.log(res)
